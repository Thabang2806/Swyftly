...
public sealed class AdminOrderPaymentTests
{
    ...
    public async Task FinanceApprover_CanRecordPaymentReconciliationReview()
    {
        ...
        using var client = factory.CreateClient();
        var seed = await SeedReconciliationCandidateDataAsync(factory);
        var financeToken = await CreateAndLoginAdminAsync(factory, client, MabuntleRoles.FinanceApprover);
    ...
    public async Task FinanceOperator_CannotRecordPaymentReconciliationReview()
    {
        ...
        using var client = factory.CreateClient();
        var seed = await SeedReconciliationCandidateDataAsync(factory);
        var operatorToken = await CreateAndLoginAdminAsync(factory, client, MabuntleRoles.FinanceOperator);
        ...
        params string[] roles)
    {
        ...
        var assignedRoles = roles.Length == 0 ? [MabuntleRoles.Admin] : roles;
    ...
    {
        private readonly string _databaseName = $"MabuntleAdminOrderPaymentTests_{Guid.NewGuid():N}";
