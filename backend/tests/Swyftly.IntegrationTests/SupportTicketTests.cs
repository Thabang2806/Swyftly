...
public sealed class SupportTicketTests
{
    ...
    public async Task SupportQueue_ReturnsSlaPriorityAssignment_AndSupportsTriageActions()
    {
        ...
        using var sellerClient = factory.CreateClient();
        using var supportClient = factory.CreateClient();
        await RegisterAndLoginAsync(sellerClient, "seller-support-queue@example.test", SwyftlyRoles.Seller);
        var supportToken = await CreateAndLoginUserInRoleAsync(factory, supportClient, "support-queue@example.test", SwyftlyRoles.SupportAgent);
    ...
    public async Task Claim_ReturnsConflictWhenAnotherSupportAgentOwnsTicket_UnlessAdminForcesOverride()
    {
        ...
        using var adminClient = factory.CreateClient();
        await RegisterAndLoginAsync(buyerClient, "buyer-claim@example.test", SwyftlyRoles.Buyer);
        firstSupportClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
            "Bearer",
            await CreateAndLoginUserInRoleAsync(factory, firstSupportClient, "support-one@example.test", SwyftlyRoles.SupportAgent));
        secondSupportClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
            "Bearer",
            await CreateAndLoginUserInRoleAsync(factory, secondSupportClient, "support-two@example.test", SwyftlyRoles.SupportAgent));
        adminClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
            "Bearer",
            await CreateAndLoginUserInRoleAsync(factory, adminClient, "support-admin@example.test", SwyftlyRoles.Admin));
