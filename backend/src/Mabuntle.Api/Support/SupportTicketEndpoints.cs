...
namespace Mabuntle.Api.Support;
...
public static class SupportTicketEndpoints
{
    ...
    public static IEndpointRouteBuilder MapSupportTicketEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        var supportGroup = app.MapGroup("/api/support/tickets")
            ...
            .RequireAuthorization(MabuntlePolicies.SupportAgentOnly);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await AddCustomerMessageAsync(ticketId, request, principal, MabuntleRoles.Buyer, buyerId: buyer.Id, sellerId: null, dbContext, timeProvider, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await AddCustomerMessageAsync(ticketId, request, principal, MabuntleRoles.Seller, buyerId: null, sellerId: seller.Id, dbContext, timeProvider, cancellationToken);
    }
    ...
    private static bool IsSupportActorRole(string role) =>
        string.Equals(role, MabuntleRoles.SupportAgent, StringComparison.OrdinalIgnoreCase)
        || string.Equals(role, MabuntleRoles.Admin, StringComparison.OrdinalIgnoreCase)
        || string.Equals(role, MabuntleRoles.SuperAdmin, StringComparison.OrdinalIgnoreCase);

    private static IQueryable<SupportTicket> TicketQuery(MabuntleDbContext dbContext) =>
        dbContext.SupportTickets
            .Include(ticket => ticket.Messages)
            .AsNoTracking();

    private static IQueryable<SupportTicket> TicketQueryForUpdate(MabuntleDbContext dbContext) =>
    ...
    {
        if (principal.IsInRole(MabuntleRoles.SuperAdmin))
        {
            return MabuntleRoles.SuperAdmin;
        }

        return principal.IsInRole(MabuntleRoles.Admin)
            ? MabuntleRoles.Admin
            : MabuntleRoles.SupportAgent;
    }

    private static bool CanOverrideAssignment(ClaimsPrincipal principal) =>
        principal.IsInRole(MabuntleRoles.Admin) || principal.IsInRole(MabuntleRoles.SuperAdmin);
    ...
    private static bool IsKnownRequesterRole(string? role) =>
        ...
        || string.Equals(role, MabuntleRoles.Buyer, StringComparison.OrdinalIgnoreCase)
        || string.Equals(role, MabuntleRoles.Seller, StringComparison.OrdinalIgnoreCase)
