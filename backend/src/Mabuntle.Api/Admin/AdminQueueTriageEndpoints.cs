...
public static class AdminQueueTriageEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            triage.Claim(actorUserId, principal.IsInRole(MabuntleRoles.SuperAdmin), timeProvider.GetUtcNow());
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var item in request.Items.DistinctBy(item => new { item.ItemType, item.ItemId }))
        {
            ...
            try
            {
                ...
                {
                    triage.Claim(actorUserId, principal.IsInRole(MabuntleRoles.SuperAdmin), now);
                }
                else
                {
                    triage.Unclaim(now);
