# Swyftly Detailed UX Flow and Screen Inventory
...
**Companion files:** `swyftly_screen_inventory.csv` contains a detailed inventory of 137 screens/states.

---

## 1. Purpose

This document turns the Swyftly product plan into a detailed UX flow and screen inventory. It is intended for:
...
Swyftly should feel like a marketplace where a user can move from discovery to purchase, or from seller onboarding to product publishing, without feeling blocked by complexity.
...
```txt
Swyftly logo | Search | Women | Men | Jewellery | Beauty | Accessories | Sell on Swyftly | Wishlist | Cart | Account
```
...
```mermaid
flowchart TD
    A[Sell on Swyftly] --> B[Create account / login]
...
```txt
Sell on Swyftly → Register → Verify email → Seller onboarding checklist
```
...
**Goal:** Help buyers quickly understand Swyftly, search products, and enter relevant categories.
...
- Explain that results are based on available Swyftly products.
...
```txt
Read docs/ux/swyftly_detailed_ux_flows_and_screen_inventory.md and implement the Angular shell for the Seller Product Creation Wizard screens S-013 to S-020.
...
For Swyftly to feel crisp and seamless, prioritise these first:
