...
const apiBaseUrl = (
  process.env.MABUNTLE_API_BASE_URL
  ?? process.env.SWYFTLY_API_BASE_URL
  ?? process.env.MABUNTLE_WEB_API_BASE_URL
...
if (!apiBaseUrl) {
  throw new Error('Set MABUNTLE_API_BASE_URL to the production API origin before building for Cloudflare Pages.');
}

const url = new URL(apiBaseUrl);
if (url.protocol !== 'https:' || url.hostname === 'localhost' || url.hostname.endsWith('.example')) {
  throw new Error('MABUNTLE_API_BASE_URL must be an external HTTPS production API origin.');
}
