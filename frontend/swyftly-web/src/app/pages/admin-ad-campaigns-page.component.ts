    ...
    <section class="page admin-review hf-admin-console">
      <app-workspace-shell>
        ...
        <app-page-header
          ...
          heading="Ad campaign operations"
        ...
        <div class="hf-metric-grid">
          @for (metric of campaignMetrics(); track metric.label) {
            <app-metric-tile
              [label]="metric.label"
              [value]="metric.value"
              [badge]="metric.badge"
        ...
        @if (isLoading()) {
          <div class="route-card">Loading ad campaign records...</div>
        } @else {
          @if (errorMessage()) {
          ...
          @if (campaigns().length === 0 && !errorMessage()) {
            <app-empty-state
              eyebrow="Clear"
              heading="No ad campaigns found"
              message="Adjust the view or filters to inspect campaign records beyond the pending review queue."
            />
          } @else {
            <div class="hf-admin-review-layout">
              <div class="hf-admin-queue-card">
                ...
                <div class="admin-table admin-moderation-table" role="table" aria-label="Ad campaign operational queue">
                  ...
                  @for (campaign of campaigns(); track campaign.adCampaignId) {
                    <div
                      ...
                      role="row"
                      [class.active]="selectedCampaign()?.adCampaignId === campaign.adCampaignId"
                    ...
                    >
                      ...
                      <span role="cell">
                        ...
                        <small>{{ campaign.sellerVerificationStatus ?? campaign.sellerId }}</small>
                      </span>
                      <span role="cell">
                        <strong>
                        ...
                        <small>{{ campaign.startsAtUtc | date:'mediumDate' }} - {{ campaign.endsAtUtc | date:'mediumDate' }}</small>
                      </span>
                      <span role="cell">
                        <app-status-badge [label]="campaign.status" [tone]="campaignStatusTone(campaign.status)" />
                        <app-status-badge [label]="slaLabel(campaign.slaStatus)" [tone]="slaTone(campaign.slaStatus)" />
                        <small>{{ campaign.priority }} / {{ campaign.assignedToDisplayName ?? 'Unassigned' }}</small>
                        <small>Age {{ campaign.ageHours ?? 0 }}h / due {{ campaign.slaDueAtUtc | date:'short' }}</small>
              ...
              @if (selectedCampaign()) {
                <aside class="hf-admin-evidence-panel">
                  <div class="hf-admin-card-heading">
                    ...
                    <app-status-badge [label]="selectedCampaign()!.status" [tone]="campaignStatusTone(selectedCampaign()!.status)" />
                  </div>

                  <div class="hf-admin-storefront-visual">
                    <span>{{ selectedCampaign()!.campaignType }}</span>
                    <strong>{{ selectedCampaign()!.productCount }} promoted product{{ selectedCampaign()!.productCount === 1 ? '' : 's' }}</strong>
                  </div>

                  <div class="hf-admin-summary-panel">
                    <strong>Campaign context</strong>
                    <span>{{ selectedCampaign()!.sellerDisplayName ?? 'Unnamed seller' }} / {{ selectedCampaign()!.sellerVerificationStatus ?? 'Unknown seller status' }}</span>
                    <span>
                      Budget:
                      @if (selectedCampaign()!.totalBudget !== null) {
                        {{ selectedCampaign()!.totalBudget | currency:(selectedCampaign()!.currency ?? 'ZAR'):'symbol-narrow' }}
                      } @else {
                        Not set
                      }
                    </span>
                    <span>Submitted {{ selectedCampaign()!.submittedAtUtc ? (selectedCampaign()!.submittedAtUtc | date:'medium') : 'not submitted' }}</span>
                  </div>
                  ...
                  <app-admin-queue-triage-panel
                    itemType="AdCampaign"
                    [itemId]="selectedCampaign()!.adCampaignId"
                    [summary]="selectedCampaign()!"
                    (triageChanged)="loadCampaigns()"
                  />
                </aside>
              }
            </div>
...
export class AdminAdCampaignsPageComponent implements OnInit {
  ...
  protected readonly campaigns = signal<AdminAdCampaignOperationalSummaryResponse[]>([]);
  ...
  protected readonly selectedCampaign = computed(() => {
    ...
    return campaigns.find(campaign => campaign.adCampaignId === selectedCampaignId) ?? campaigns[0];
  });

  protected readonly totalPages = computed(() => Math.max(1, Math.ceil(this.totalCount() / this.pageSize())));

  protected readonly campaignMetrics = computed(() => {
    const pending = this.statusCounts().find(count => count.status === 'PendingReview')?.count ?? 0;
    const active = this.statusCounts().find(count => count.status === 'Active')?.count ?? 0;
    const rejected = this.statusCounts().find(count => count.status === 'Rejected')?.count ?? 0;
    const visibleBudget = this.campaigns().reduce((total, campaign) => total + (campaign.totalBudget ?? 0), 0);

    return [
      { label: 'Pending review', value: pending.toString(), badge: 'Queue', tone: pending > 0 ? 'warning' as StatusBadgeTone : 'success' as StatusBadgeTone },
      { label: 'Active campaigns', value: active.toString(), badge: 'Live', tone: 'success' as StatusBadgeTone },
      ...
      { label: 'Visible budget', value: visibleBudget.toLocaleString('en-ZA', { style: 'currency', currency: 'ZAR' }), badge: 'Rows', tone: 'accent' as StatusBadgeTone }
    ];
  });
  ...
  protected selectCampaign(campaign: AdminAdCampaignOperationalSummaryResponse): void {
    this.selectedCampaignId.set(campaign.adCampaignId);
  }

  protected isQueueItemSelected(campaignId: string): boolean {
    return this.selectedQueueItemIds().includes(campaignId);
  }

  protected toggleQueueItem(campaignId: string, isSelected: boolean): void {
  ...
  protected campaignStatusTone(status: string): StatusBadgeTone {
  ...
  protected async loadCampaigns(): Promise<void> {
    ...
    try {
      ...
      const response = await this.adminAdCampaignService.getCampaigns({
        view: this.view(),
      ...
      this.selectedCampaignId.set(response.items[0]?.adCampaignId ?? null);
      this.selectedQueueItemIds.set(this.selectedQueueItemIds().filter(id => response.items.some(item => item.adCampaignId === id)));
    } catch (error) {
      this.errorMessage.set(getApiErrorMessage(error));
    } finally {
  ...
  private async bulkTriage(action: 'Claim' | 'SetPriority', priority?: 'High'): Promise<void> {
    ...
    try {
      await this.adminQueueTriageService.bulkTriage({
        action,
        priority,
        items: ids.map(itemId => ({ itemType: 'AdCampaign', itemId }))
      });
      await this.loadCampaigns();
    } catch (error) {
      this.errorMessage.set(getApiErrorMessage(error));
  ...
  private async loadSavedViews(): Promise<void> {
    const views = await this.adminModerationQueueService.getSavedViews('Ads');
